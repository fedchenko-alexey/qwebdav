...
{
    return nullptr == m_reply ? true : m_reply->isFinished();
}
...
#ifdef DEBUG_WEBDAV
            qDebug() << "   Reply has error. Error:" << m_reply->errorString()
                     << "Code:" << m_reply->error();
#endif
        } else {
...
void QWebdavDirParser::replyDeleteLater(QNetworkReply *reply)
{
    ...
    qDebug() << "QWebdavDirParser::replyDeleteLater()      reply->isFinished() == "
             << reply->isFinished();
    qDebug() << "QWebdavDirParser::replyDeleteLater()      reply->bytesAvailable() == "
             << reply->bytesAvailable();
    qDebug() << "QWebdavDirParser::replyDeleteLater()      reply->bytesToWrite() == "
             << reply->bytesToWrite();
#endif
...
void QWebdavDirParser::parseMultiResponse(const QByteArray &data)
{
    ...
    for (auto n = multiResponse.documentElement().firstChild(); !n.isNull(); n = n.nextSibling()) {
        if (m_abort) {
            return;
        }
        ...
        QString responseName = QUrl::fromPercentEncoding(
            thisResponse.namedItem("href").toElement().text().toUtf8());
...
void QWebdavDirParser::parseResponse(const QDomElement &dom)
{
    ...
    auto href = dom.namedItem("href").toElement();
    if (href.isNull()) {
        return;
    }
...
void QWebdavDirParser::davParsePropstats(const QString &path, const QDomNodeList &propstats)
{
    ...
    // NOTE: непонятно зачем удаляется часть пути. Пока отключено.
    // path_.remove(0, m_webdav->rootPath().size());

#ifdef QWEBDAVITEM_EXTENDED_PROPERTIES
    m_dirList.append(QWebdavItem(path_,
                                 name,
                                 ext,
                                 dirOrFile,
                                 lastModified,
                                 ...
                                 source));
#else
    m_dirList.append(QWebdavItem(path_, name, ext, dirOrFile, lastModified, size));
#endif
}
...
int QWebdavDirParser::codeFromResponse(const QString &response)
{
    ...
    auto str = response.mid(firstSpace + 1, secondSpace - firstSpace - 1);
    return str.toInt();
}
